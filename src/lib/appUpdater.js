  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "voidRoute")
        : path.join(os.homedir(), ".voidRoute"),
      "mitm",
      ".mitm.pid"
    );
...
// Collect PIDs of all voidRoute-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from voidRoute install dir or wrapper cli.js
        const isAppProcess = lower.includes("voidRoute") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("voidRoute") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "voidRoute");
  }
  return path.join(os.homedir(), ".voidRoute");
}
...
// Resolve npx/voidRoute binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx voidRoute` — works regardless of global bin path changes after npm i -g
