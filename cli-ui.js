...
import { HermesAdapter } from './src/lib/cli-config/index.js';
...
export async function manageCliTools(port) {
  ...
  function detectStatus(tool) {
    try {
      ...
      if (tool === 'codex') {
        const cxConfig = path.join(os.homedir(), '.codex', 'config.toml');
        if (!fs.existsSync(cxConfig)) return null;
        const content = fs.readFileSync(cxConfig, 'utf8');
      ...
      if (tool === 'hermes') {
        return new HermesAdapter().detectStatus();
      }
    } catch (e) { }
    return null;
  }
      ...
      if (action === 'reset') {
        ...
        try {
          ...
          if (modelIds.length > 0) {
            ...
            if (fs.existsSync(cxCache)) {
              try { existing = JSON.parse(fs.readFileSync(cxCache, 'utf8')); } catch (e) {}
            }

            const now = Math.floor(Date.now() / 1000);
            const voidRouteModels = modelIds.map(id => {
            ...
            for (const m of voidRouteModels) {
              if (!mergedData.find(x => x.id === m.slug)) {
                mergedData.push({ id: m.slug, object: 'model', created: now, owned_by: 'voidRoute' });
              }
              if (!mergedModels.find(x => x.slug === m.slug)) {
                mergedModels.push(m);
              }
            }
            ...
            fs.writeFileSync(cxCache, JSON.stringify(existing, null, 2));
          }

          console.log(chalk.green(`  ✅ Codex configured with OpenCodex multi-model routing via voidRoute.`));
          if (modelIds.length > 1) {
            console.log(chalk.cyan(`  📦 ${modelIds.length} models cached in ~/.codex/models_cache.json.`));
          }
